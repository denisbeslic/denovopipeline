...
#~     ion_mass_list = z_ions + y_ions + x_ions + c_ions + b_ions + a_ions
  ...
  # ion locations
  location_sub50 = np.rint(ion_mass*data_utils.SPECTRUM_RESOLUTION).astype(np.int32)
  location_sub50 -= (data_utils.WINDOW_SIZE // 2)
  #
  location_plus50 = location_sub50 + data_utils.WINDOW_SIZE
  #
  ...
  #~ for aa_id in ([LAST_LABEL] + range(3,data_utils.vocab_size)):
    #~ for ion_id in xrange(data_utils.num_ion):
      ...
      #~ #
      #~ if (location_sub50 > 0):
        #~ candidate_intensity[aa_id,ion_id] = spectrum_original[location_sub50:location_sub50+data_utils.WINDOW_SIZE]
...
def process_spectrum(spectrum_mz_list, spectrum_intensity_list, peptide_mass):
  ...
  neutral_mass_location = np.rint(neutral_mass*data_utils.SPECTRUM_RESOLUTION).astype(np.int32)
...
#~     spectrum_holder_view[neutral_mass_location_view[index]] += norm_intensity_view[index]
  ...
  complement_mass_location = np.rint(complement_mass*data_utils.SPECTRUM_RESOLUTION).astype(np.int32)
...
#~   cdef int index
  ...
  # peptide_mass
  #
  spectrum_original_forward[int(round(peptide_mass*data_utils.SPECTRUM_RESOLUTION))] = 1.0
  #
  spectrum_original_backward[int(round(peptide_mass*data_utils.SPECTRUM_RESOLUTION))] = 1.0

  # N-terminal, b-ion, peptide_mass_C
  #
  # append N-terminal
  mass_N = data_utils.mass_N_terminus - data_utils.mass_H
  spectrum_holder[int(round(mass_N*data_utils.SPECTRUM_RESOLUTION))] = 1.0
  #
  # append peptide_mass_C
  mass_C = data_utils.mass_C_terminus + data_utils.mass_H
  peptide_mass_C = peptide_mass - mass_C
  spectrum_holder[int(round(peptide_mass_C*data_utils.SPECTRUM_RESOLUTION))] = 1.0
  #
  spectrum_original_forward[int(round(peptide_mass_C*data_utils.SPECTRUM_RESOLUTION))] = 1.0

  # C-terminal, y-ion, peptide_mass_N
  #
  # append C-terminal
  mass_C = data_utils.mass_C_terminus + data_utils.mass_H
  spectrum_holder[int(round(mass_C*data_utils.SPECTRUM_RESOLUTION))] = 1.0
  #
  # append peptide_mass_N
  mass_N = data_utils.mass_N_terminus - data_utils.mass_H
  peptide_mass_N = peptide_mass - mass_N
  spectrum_holder[int(round(peptide_mass_N*data_utils.SPECTRUM_RESOLUTION))] = 1.0
  #
  spectrum_original_backward[int(round(peptide_mass_N*data_utils.SPECTRUM_RESOLUTION))] = 1.0
