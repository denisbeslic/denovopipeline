...
def create_train_model(
  ...
  src_file = []
  for file_name in hparams.src:
    src_file.append("%s%s" % (file_name, hparams.src_suffix ))
...
def create_emb_for_decoder(#share_vocab,
                           #src_vocab_size,
                           tgt_vocab_size,
                           #src_embed_size,
                           tgt_embed_size,
  ...
  if tgt_embed_file and partitioner:
    raise ValueError(
...
def create_or_load_model(model, model_dir, session, name):
  ...
  if latest_ckpt:
    ...
    session.run(tf.tables_initializer())
    compute_dp_array(model.suffix_dp_table).run(session=session)
...
def _refine_dp_array(original_dp_array, max_dp_array_size, inv_dp_resolution, error_window):
  utils.print_out("  Refining DP array to allow %f Da error window" % (error_window))
  
  refined_dp_array = np.zeros(max_dp_array_size, dtype=bool)
  # error_window = 0.2
  error_window_size = int(round(error_window * inv_dp_resolution))

  for i in range(max_dp_array_size):
      start = max(0, i - error_window_size)
      end = min(max_dp_array_size, i + error_window_size)
      if np.sum(original_dp_array[start:end]) > 0:
          refined_dp_array[i] = True

  return refined_dp_array

def compute_dp_array(dp_array_tensor):
  start_time = time.time()
  utils.print_out("  Computing DP array of size %f with resolution %f" % (input_config.max_dp_sz, input_config.dp_resolution))
  max_dp_array_size = input_config.max_dp_array_size
  inv_dp_resolution = input_config.inv_dp_resolution
  
  dp = np.zeros(max_dp_array_size, dtype=bool)
  dp[0] = True
  
  for mass in input_config.mass_ID_np:
      mass = int(round(mass * inv_dp_resolution))
      # print(mass)
      for i in range(max_dp_array_size):
          if dp[i] and i + mass < max_dp_array_size:
              dp[i + mass] = True

  # Refine with 0.2 Da possible error
  error_window = 0.2
  dp_table_value = _refine_dp_array(dp, max_dp_array_size, inv_dp_resolution, error_window)
  
  utils.print_out("  created suffix DP array , time %.2fs" % (time.time() - start_time))
  # dp_array_tensor.assign(dp_table_value).op.run()
  return dp_array_tensor.assign(dp_table_value).op
...
def compute_perplexity(model, sess, name):
  ...
  Returns:
    ...
    Total loss of the eval outputs.
  """
  total_loss = 0
