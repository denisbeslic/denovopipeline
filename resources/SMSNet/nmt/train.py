...
def run_internal_eval(
  ...
  dev_src_file = "%s%s" % (hparams.dev, hparams.src_suffix)
  ...
  dev_ppl = _internal_eval(loaded_eval_model, global_step, eval_sess,
                           ...
                           summary_writer, "dev")
  test_ppl = None
  if use_test_set and hparams.test:
    test_src_file = "%s%s" % (hparams.test, hparams.src_suffix)
...
def run_external_eval(infer_model, infer_sess, model_dir, hparams,
  ...
  dev_src_file = "%s%s" % (hparams.dev, hparams.src_suffix)
  dev_tgt_file = "%s%s" % (hparams.dev, hparams.tgt)
  ...
  if use_test_set and hparams.test:
    test_src_file = "%s%s" % (hparams.test, hparams.src_suffix)
    test_tgt_file = "%s%s" % (hparams.test, hparams.tgt)
...
def train(hparams, scope=None, target_session=""):
  ...
  # For stoping inference run after the training was already finished 
  if hparams.train_fin:
    raise SystemExit
...
def _sample_decode(model, global_step, sess, hparams, iterator, src_data,
  ...
  for i in range(2):
    ...
    nmt_outputs, steps_probs, attention_summary = model.decode(sess, 1) 
    if hparams.beam_width > 0:
      # get the top translation.
      nmt_outputs = nmt_outputs[0]
      steps_probs = steps_probs[0]
    ...
    trans_step_probs = nmt_utils.get_log_probs(
        steps_probs,
        sent_id=0,
    ...
    utils.print_out(b"    prob: " + trans_step_probs)
